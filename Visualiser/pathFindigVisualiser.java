...
public class pathFindigVisualiser {
    // GENERAL VARIABLES
    ...
    // UTIL ARRAYS
    private String[] algorithms = { "Dijkstra(BFS)", "A*" }; // More Algos to be added
    ...
    JLabel obstacleL = new JLabel("Dense:");
    ...
    public static void main(String[] args) { // MAIN METHOD
        new pathFindigVisualiser();
    }

    public pathFindigVisualiser() { // CONSTRUCTOR
        clearMap();
        initialize();
    }
    ...
    private void initialize() { // INITIALIZE THE GUI ELEMENTS
        ...
        checkL.setBounds(15, space, 100, 35);
        toolP.add(checkL);
        space += buff;

        lengthL.setBounds(15, space, 100, 35);
        toolP.add(lengthL);
        space += buff;
        ...
        searchB.addActionListener(new ActionListener() { // ACTION LISTENERS
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if ((startx > -1 && starty > -1) && (finishx > -1 && finishy > -1))
                    ...
                    JOptionPane.showMessageDialog(frame, "Start/Finish point missing!");
                }
            }
        });
    ...
    class Algorithm { // ALGORITHM CLASS
        ...
        // DIJKSTRA WORKS BY PROPAGATING OUTWARDS UNTIL IT FINDS THE FINISH AND THEN
        // WORKING ITS WAY BACK TO GET THE PATH
        // IT USES A PRIORITY QUE TO KEEP TRACK OF NODES THAT IT NEEDS TO EXPLORE
        // EACH NODE IN THE PRIORITY QUE IS EXPLORED AND ALL OF ITS NEIGHBORS ARE ADDED
        // TO THE QUE
        // ONCE A NODE IS EXLPORED IT IS DELETED FROM THE QUE
        // AN ARRAYLIST IS USED TO REPRESENT THE PRIORITY QUE
        // A SEPERATE ARRAYLIST IS RETURNED FROM A METHOD THAT EXPLORES A NODES
        // NEIGHBORS
        // THIS ARRAYLIST CONTAINS ALL THE NODES THAT WERE EXPLORED, IT IS THEN ADDED TO
        // THE QUE
        // A HOPS VARIABLE IN EACH NODE REPRESENTS THE NUMBER OF NODES TRAVELED FROM THE
        // START
        ...
        // A STAR WORKS ESSENTIALLY THE SAME AS DIJKSTRA CREATING A PRIORITY QUE AND
        // PROPAGATING OUTWARDS UNTIL IT FINDS THE END
        ...
        // THIS HEURISTIC IS BUILT IN BY SORTING THE QUE ACCORDING TO HOPS PLUS DISTANCE
        // UNTIL THE FINISH
        public void AStar() {
        ...
        public ArrayList<Node> sortQue(ArrayList<Node> sort) { // SORT PRIORITY QUE
            ...
            while (c < sort.size()) {
                ...
                for (int i = c + 1; i < sort.size(); i++) {
                    if (sort.get(i).getEuclidDist() + sort.get(i).getHops() < sort.get(sm).getEuclidDist()
                            + sort.get(sm).getHops())
                        sm = i;
                }
                if (c != sm) {
        ...
        public ArrayList<Node> exploreNeighbors(Node current, int hops) { // EXPLORE NEIGHBORS
            ...
            for (int a = -1; a <= 1; a++) {
                for (int b = -1; b <= 1; b++) {
                    ...
                    if ((xbound > -1 && xbound < cells) && (ybound > -1 && ybound < cells)) { // MAKES SURE THE NODE IS
                                                                                              // NOT OUTSIDE THE GRID
                        Node neighbor = map[xbound][ybound];
                        if ((neighbor.getHops() == -1 || neighbor.getHops() > hops) && neighbor.getType() != 2) {
                            // CHECKS IF NODE IS NOT A WALL AND IS BEING EXPLORED
